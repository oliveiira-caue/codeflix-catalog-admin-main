...
class GenreViewSet(viewsets.ViewSet):
    ...
    def update(self, request: Request, pk: UUID = None) -> Response:
        serializer = UpdateGenreInputSerializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        ...
        try:
            use_case.execute(UpdateGenre.Input(
                ...
                categories=serializer.validated_data.get("categories", set()),
            ))
    ...
    def destroy(self, request: Request, pk: UUID = None) -> Response:
        ...
        try:
            use_case.execute(DeleteGenre.Input(**request_data.validated_data))
        except GenreNotFound:
            return Response(
